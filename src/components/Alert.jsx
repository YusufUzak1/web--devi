import { useState } from 'react'

/**
 * Alert Bileşeni
 * Tipler: info, success, warning, error
 * Kapatılabilir (dismissible)
 */
export default function Alert({
    children,
    type = 'info',
    dismissible = false,
    onClose,
    className = '',
}) {
    const [visible, setVisible] = useState(true)

    if (!visible) return null

    const typeConfig = {
        info: {
            bg: 'bg-info/10 border-info/30 dark:bg-info/15 dark:border-info/40',
            text: 'text-info dark:text-info',
            icon: (
                <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                </svg>
            ),
        },
        success: {
            bg: 'bg-success/10 border-success/30 dark:bg-success/15 dark:border-success/40',
            text: 'text-success dark:text-success',
            icon: (
                <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
            ),
        },
        warning: {
            bg: 'bg-warning/10 border-warning/30 dark:bg-warning/15 dark:border-warning/40',
            text: 'text-warning dark:text-warning',
            icon: (
                <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                </svg>
            ),
        },
        error: {
            bg: 'bg-error/10 border-error/30 dark:bg-error/15 dark:border-error/40',
            text: 'text-error dark:text-error',
            icon: (
                <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                </svg>
            ),
        },
    }

    const config = typeConfig[type]

    const handleClose = () => {
        setVisible(false)
        onClose?.()
    }

    return (
        <div
            className={`flex items-start gap-3 px-4 py-3 rounded-md border ${config.bg} ${config.text} ${className}`}
            role="alert"
        >
            {config.icon}
            <div className="flex-1 text-sm font-medium">{children}</div>
            {dismissible && (
                <button
                    onClick={handleClose}
                    className="shrink-0 p-0.5 rounded hover:bg-white/10 transition-colors cursor-pointer"
                    aria-label="Bildirimi kapat"
                >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                    </svg>
                </button>
            )}
        </div>
    )
}
